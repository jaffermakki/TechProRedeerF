...
def _send_via_brevo_api(api_key: str, from_email: str, from_name: str, to_email: str,
                         subject: str, html_content: str = None, text_content: str = None,
                         attachment_name: str = None, attachment_content: bytes = None) -> tuple[bool, str]:
...
def send_backup_email(db, to_email: str, backup_bytes: bytes, filename: str, get_setting) -> tuple[bool, str]:
    """Emails a full data backup as an attachment — used both by the
    'Send backup now' test button and the automatic weekly job. Shares
    the same SMTP-vs-Brevo-API fork every other send function here uses,
    since attachments work differently on each: Brevo takes base64 bytes
    directly in the JSON payload, while SMTP needs a real MIME multipart
    message with the file as its own part."""
    shop_name = get_setting(db, "shop_name", "Your Shop")
    subject = f"{shop_name} — Data Backup ({datetime.utcnow().strftime('%Y-%m-%d')})"
    body = (
        f"Attached is a full backup of {shop_name}'s CRM data — products, customers, "
        f"repairs, invoices, and settings — as of {datetime.utcnow().strftime('%B %d, %Y')}.\n\n"
        f"Keep this somewhere safe (a separate email folder, cloud storage, wherever you'd "
        f"actually look for it if you needed it). To restore it, go to Settings → Data → "
        f"Restore from Backup and upload this file.\n\n"
        f"Note: restoring replaces current products, customers, repairs, and settings — "
        f"it does not touch existing invoices."
    )
    ...
    if get_setting(db, "email_method", "smtp") == "brevo_api":
        ...
        if not (api_key and from_email):
            return False, "Brevo API is not fully configured — set the API key and From address in Settings → Notifications."
        ok, detail = _send_via_brevo_api(api_key, from_email, from_name or shop_name, to_email, subject,
                                          text_content=body, attachment_name=filename, attachment_content=backup_bytes)
        if ok:
            return True, "Backup " + detail.lower() + " Check that it actually arrived — spam folders sometimes catch attachments."
        return False, detail
    ...
    part = MIMEApplication(backup_bytes, Name=filename)
    ...
    if ok:
        warning = _from_address_warning(user, from_addr)
        base = "Backup emailed successfully."
        return True, base + warning if warning else base + " Check that it actually arrived — spam folders sometimes catch attachments."
    return False, detail
