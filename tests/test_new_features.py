...
def test_trade_in_gets_a_sequential_receipt_number(owner_client, db_session):
    owner_client.post("/trade-ins/add", data={
        "customer_id": "", "new_customer_name": "", "new_customer_phone": "",
        "device": "Numbered Trade Phone", "imei": "", "condition": "Fair",
        "offered_amount": "40.00", "payout_method": "cash", "notes": "",
    })
    trade_in = db_session.query(TradeIn).filter(TradeIn.device == "Numbered Trade Phone").first()
    assert trade_in.number.startswith("TR-")


def test_trade_in_numbers_increment_across_two_trade_ins(owner_client, db_session):
    owner_client.post("/trade-ins/add", data={
        "customer_id": "", "new_customer_name": "", "new_customer_phone": "",
        "device": "First Increment Phone", "imei": "", "condition": "",
        "offered_amount": "10.00", "payout_method": "cash", "notes": "",
    })
    owner_client.post("/trade-ins/add", data={
        "customer_id": "", "new_customer_name": "", "new_customer_phone": "",
        "device": "Second Increment Phone", "imei": "", "condition": "",
        "offered_amount": "10.00", "payout_method": "cash", "notes": "",
    })
    first = db_session.query(TradeIn).filter(TradeIn.device == "First Increment Phone").first()
    second = db_session.query(TradeIn).filter(TradeIn.device == "Second Increment Phone").first()
    assert first.number != second.number


def test_trade_in_print_page_shows_device_and_amount(owner_client, db_session):
    owner_client.post("/trade-ins/add", data={
        "customer_id": "", "new_customer_name": "Print Test Customer", "new_customer_phone": "555-6099",
        "device": "Printable Test Device", "imei": "999888777", "condition": "Like new",
        "offered_amount": "75.50", "payout_method": "store_credit", "notes": "",
    })
    trade_in = db_session.query(TradeIn).filter(TradeIn.device == "Printable Test Device").first()

    resp = owner_client.get(f"/trade-ins/{trade_in.id}/print")
    assert resp.status_code == 200
    assert "Printable Test Device" in resp.text
    assert "999888777" in resp.text
    assert "75.50" in resp.text
    assert "Print Test Customer" in resp.text
    assert trade_in.number in resp.text


def test_trade_in_print_page_shows_walk_in_for_no_customer(owner_client, db_session):
    owner_client.post("/trade-ins/add", data={
        "customer_id": "", "new_customer_name": "", "new_customer_phone": "",
        "device": "Walk In Print Device", "imei": "", "condition": "",
        "offered_amount": "5.00", "payout_method": "cash", "notes": "",
    })
    trade_in = db_session.query(TradeIn).filter(TradeIn.device == "Walk In Print Device").first()
    resp = owner_client.get(f"/trade-ins/{trade_in.id}/print")
    assert resp.status_code == 200
    assert "Walk-in Customer" in resp.text


def test_trade_in_print_with_bogus_id_redirects_not_crashes(owner_client):
    resp = owner_client.get("/trade-ins/this-id-does-not-exist/print", follow_redirects=False)
    assert resp.status_code == 303
    assert resp.headers["location"] == "/trade-ins"


def test_trade_in_print_requires_login(anon_client, owner_client, db_session):
    owner_client.post("/trade-ins/add", data={
        "customer_id": "", "new_customer_name": "", "new_customer_phone": "",
        "device": "Login Required Print Device", "imei": "", "condition": "",
        "offered_amount": "5.00", "payout_method": "cash", "notes": "",
    })
    trade_in = db_session.query(TradeIn).filter(TradeIn.device == "Login Required Print Device").first()
    resp = anon_client.get(f"/trade-ins/{trade_in.id}/print", follow_redirects=False)
    assert resp.status_code == 303
    assert resp.headers["location"] == "/login"


def test_trade_in_print_link_appears_on_the_list_page(owner_client, db_session):
    owner_client.post("/trade-ins/add", data={
        "customer_id": "", "new_customer_name": "", "new_customer_phone": "",
        "device": "Link Check Device", "imei": "", "condition": "",
        "offered_amount": "5.00", "payout_method": "cash", "notes": "",
    })
    trade_in = db_session.query(TradeIn).filter(TradeIn.device == "Link Check Device").first()
    resp = owner_client.get("/trade-ins")
    assert f"/trade-ins/{trade_in.id}/print" in resp.text


def test_trade_in_print_falls_back_gracefully_when_number_is_blank(owner_client, db_session):
    """Covers a trade-in from before this feature existed — created
    directly here (bypassing the route, which always sets a number now)
    to simulate a pre-migration record with number="". The print page
    must still render something reasonable, not crash or show a bare
    'TR-' with nothing after it."""
    old_trade_in = TradeIn(
        number="", device="Pre-Migration Phone", imei="", condition="Used",
        offered_amount=25.00, payout_method="cash", staff_name="Owner",
    )
    db_session.add(old_trade_in)
    db_session.commit()

    resp = owner_client.get(f"/trade-ins/{old_trade_in.id}/print")
    assert resp.status_code == 200
    assert "Pre-Migration Phone" in resp.text
    assert old_trade_in.id[:8].upper() in resp.text


def test_last_trade_in_shows_a_prominent_print_banner_right_after_adding(owner_client, db_session):
    """Distinct from the always-present per-row print link — this is the
    one-time call-to-action right after completing a trade-in, so
    handing the customer a receipt doesn't require hunting through the
    table for the row that was just added.

    Checked on the POST's own (auto-followed) response — TestClient
    follows redirects by default, so that response IS the page the user
    actually lands on, same as a real browser after submitting the form."""
    resp = owner_client.post("/trade-ins/add", data={
        "customer_id": "", "new_customer_name": "", "new_customer_phone": "",
        "device": "Banner Check Device", "imei": "", "condition": "",
        "offered_amount": "5.00", "payout_method": "cash", "notes": "",
    })
    trade_in = db_session.query(TradeIn).filter(TradeIn.device == "Banner Check Device").first()
    assert "Print This Receipt" in resp.text
    assert f"/trade-ins/{trade_in.id}/print" in resp.text


def test_print_banner_does_not_linger_on_a_later_page_load(owner_client, db_session):
    """The banner is popped from the session, not just conditionally
    shown — confirms it appears once (on the redirect the form submit
    itself lands on) and is gone on any page load after that, not just
    coincidentally absent because it was never actually shown."""
    first_view = owner_client.post("/trade-ins/add", data={
        "customer_id": "", "new_customer_name": "", "new_customer_phone": "",
        "device": "One Time Banner Device", "imei": "", "condition": "",
        "offered_amount": "5.00", "payout_method": "cash", "notes": "",
    })
    assert "Print This Receipt" in first_view.text

    second_load = owner_client.get("/trade-ins")
    assert "Print This Receipt" not in second_load.text


def test_trade_ins_list_shows_the_reference_number_column(owner_client, db_session):
    owner_client.post("/trade-ins/add", data={
        "customer_id": "", "new_customer_name": "", "new_customer_phone": "",
        "device": "Number Column Check Device", "imei": "", "condition": "",
        "offered_amount": "5.00", "payout_method": "cash", "notes": "",
    })
    trade_in = db_session.query(TradeIn).filter(TradeIn.device == "Number Column Check Device").first()
    resp = owner_client.get("/trade-ins")
    assert trade_in.number
    assert trade_in.number in resp.text
...
# ── SEND TEST SMS (Settings → Notifications) ────────────────────────
# Mirrors the existing "Send Test Email" pattern — lets an owner confirm
# Twilio is actually configured correctly without needing a real repair
# or invoice to trigger a text.

def test_send_test_sms_dispatches_with_the_entered_phone_number(owner_client, db_session):
    from unittest.mock import patch
    with patch("app.main.send_sms") as mock_send:
        mock_send.return_value = (True, "SMS sent successfully.")
        resp = owner_client.post("/settings/send-test-sms", data={"to_phone": "+14165550100"},
                                  follow_redirects=False)
    assert resp.status_code == 303
    mock_send.assert_called_once()
    args, kwargs = mock_send.call_args
    assert "+14165550100" in args


def test_send_test_sms_shows_the_result_as_a_flash_message(owner_client, db_session):
    from unittest.mock import patch
    with patch("app.main.send_sms") as mock_send:
        mock_send.return_value = (False, "SMS is not configured — go to Settings → Notifications to set up Twilio.")
        owner_client.post("/settings/send-test-sms", data={"to_phone": "+14165550100"}, follow_redirects=False)
    page = owner_client.get("/settings")
    assert "not configured" in page.text.lower()


def test_send_test_sms_requires_owner_role(manager_client):
    resp = manager_client.post("/settings/send-test-sms", data={"to_phone": "+14165550100"})
    assert resp.status_code == 403
